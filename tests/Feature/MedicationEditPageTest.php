...
class MedicationEditPageTest extends TestCase
{
    ...
    public function testHasForm()
    {
        ...
        $response->assertSee('<input type="checkbox" name="stat" id="med-stat" value="1">');
    ...
    public function testHasStatChecked()
    {
        $user = factory(\App\User::class)->create();
        $medication = factory(\App\Medication::class)->create();
        $medication->stat = 1;
        $medication->update();
        $response = $this->actingAs($user)->get('/medications/' . $medication->medication_id . '/edit');
        $response->assertSee('<input type="checkbox" name="stat" id="med-stat" value="1" checked>');
    }
}
